function [ results, misc ] = bac1_pmala(y, H, option, init)
...
chain_arate = [];
tic
[chain_x, chain_mu, chain_sigma2, chain_arate, chain_delta] = bac1_pmala_c(...
    y, H, Nit, ...
    x_init, mu_init, sigma2_init, ...
    sample_x, sample_mu, sample_sigma2, ...
    a_mu, b_mu, a_sigma2, b_sigma2, ...
    thinning, window, delta, ...
    randi(10^6));
misc.time = toc;
misc.type = 'bac1_pmala';

results.x_all      = chain_x;
results.sigma2_all = chain_sigma2;
results.mu_all     = chain_mu;
results.arate_all  = chain_arate;
results.delta_all  = chain_delta;
